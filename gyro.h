...
int readWord_2c(int handle, int Reg, int Offset) {
    int val = (readWord(handle,Reg))-Offset;
    if (val >= 32768) {
        return -(65535 - val) ;
    }else {
        return val;
...
float gyroskop_xout(int gyrohd) {
    return (readWord_2c(gyrohd,0x43,GX_Offset)/GyroSens);
}
float gyroskop_yout(int gyrohd) {
    return (readWord_2c(gyrohd,0x45,GY_Offset)/GyroSens);
}
float gyroskop_zout(int gyrohd) {
    return (readWord_2c(gyrohd,0x47,GZ_Offset)/GyroSens);
}
float accel_xout(int gyrohd) {
    return (readWord_2c(gyrohd,0x3b,AX_Offset)/AccelSens);
}
float accel_yout(int gyrohd) {
    return (readWord_2c(gyrohd,0x3d,AY_Offset)/AccelSens);
}
float accel_zout(int gyrohd) {
    return (readWord_2c(gyrohd,0x3f,AZ_Offset)/AccelSens);
}
int get_y_rotation(int gyrohd) {
    int x,y,z, radians;
...
int set_gyro_y(int gyrohd) {
    for (int i = 0; i< 100; i++) {
        GY_Offset +=readWord_2c(gyrohd,0x45,0);
    }
    GY_Offset = (int) GY_Offset/100;
    return GY_Offset;
}
int set_gyro_x(int gyrohd) {
    for (int i = 0; i< 100; i++) {
        GX_Offset += readWord_2c(gyrohd,0x43,0);
    }
    GX_Offset = (int) GX_Offset/100;
    return GX_Offset;
}
int set_gyro_z(int gyrohd) {
    for (int i = 0; i< 100; i++) {
        GZ_Offset += readWord_2c(gyrohd,0x47,0);
    }
    GZ_Offset = (int) GZ_Offset/100;
    return GZ_Offset;
}
int set_accel_z(int gyrohd) {
    for (int i = 0; i< 100; i++) {
        AZ_Offset += readWord_2c(gyrohd,0x3f,0);
    }
    AZ_Offset = (int) AZ_Offset/100;
    return AZ_Offset;
}
int set_accel_y(int gyrohd) {
    for (int i = 0; i< 100; i++) {
        AY_Offset += readWord_2c(gyrohd,0x3d,0);
    }
    AY_Offset = (int) AY_Offset/100;
    return AY_Offset;
}
int set_accel_x(int gyrohd) {
    for (int i = 0; i< 100; i++) {
        AX_Offset += readWord_2c(gyrohd,0x3b,0);
    }
    AX_Offset = (int) AX_Offset/100;
    return AX_Offset;
}
