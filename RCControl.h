...
//RC-Channel 1	(analog Stick)
	steering = map(RC_Channel[0],RCmin,RCmax,10,-10);
//RC-Channel 2	(analog Stick)

//RC-Channel 3	(analog Stick)
	if (EncoderMode) {
		Spin_Target = (map(RC_Channel[2],RCmin,RCmax,0,(int)SPIN_MAX));
	}else{
		throttle = (map(RC_Channel[2], RCmin,RCmax,0,(int)THROTTLE_MAX));
	}
//RC-Channel 4	(analog Stick)

//RC-Channel 5	(analog rotary encoder)
	ACCELERATION	= (map(RC_Channel[4], RCmin,RCmax,0,(int)THROTTLE_MAX));
//RC-Channel 6	(analog rotary encoder)

//RC-Channel 7	(2-way switch)
if (Anz_RC_Channels>=7) {
	if (RC_Channel[6] >= RCmid) {
		Blinker[5].dura = 1;
		Blinker[5].freq = 0;
	}
}
